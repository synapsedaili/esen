...
FONT_COLOR = "yellow"
FONT_SIZE = 95 # Okunabilirlik için ideal büyüklük
...
def apply_random_ken_burns(clip):
    # Efekt Sınırı: %65 uygulama
    if random.random() > 0.65:
        return clip

    duration = clip.duration
    zoom_factor = random.uniform(1.10, 1.20) # Çok hafif ve akıcı hareket
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    clips = []
    current_time = 0.0
    img_dur = 4.5 # Geçişler biraz daha sakinleşti
    ...
    # 3 KELİME ALTYAZI SİSTEMİ (Okunabilirlik Düzeltilmiş)
    ...
    start_t = 0.0
    for chunk in chunks:
        # Kutu boyutunu genişleterek harf ezilmesini önlüyoruz
        txt = TextClip(
            text=chunk.upper(),
            font_size=FONT_SIZE, 
            font=VIDEO_FONT, 
            color=FONT_COLOR, 
            method="caption", 
            size=(int(VIDEO_W * 0.9), 600), # Sabit geniş kutu
            text_align="center",
            stroke_color="black",
            stroke_width=3
