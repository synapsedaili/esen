...
def apply_random_ken_burns(clip):
    # EFEKT SINIRI: Görsellerin %65'inde hareket olsun, %35'i sabit kalsın (Işınlanma hissini önler)
...
def upload_to_youtube(video_path, meta, lang_code):
    try:
        ...
        if lang_code == "es":
            yt_lang = "es-ES"
            fixed_tags = [
                "Crecimiento Personal", "Mentalidad", "Datos del Cerebro", 
                "Verdades Ocultas", "Datos Psicológicos", "Psicología Social", 
                "Por qué somos así."
            ]
        else:
            yt_lang = "en-US"
            fixed_tags = [
                "Self Improvement", "Mindset", "Brain Facts", 
                "Hidden Truths", "Psychological Facts", "Social Psychology", 
                "Why We Do What We Do"
            ]

        dynamic_tags = meta.get('tags', [])
        final_tags = list(set(fixed_tags + dynamic_tags))
        ...
        body = {
            "snippet": {
                ...
                "tags": final_tags,
                "categoryId": "22",  # Kişiler ve Bloglar
        ...
        print(f"[{lang_code.upper()}] {yt_lang} ayarlarıyla yükleniyor...")
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    # 3 KELİMELİK OKUNABİLİR ALTYAZI SİSTEMİ
    ...
    for chunk in chunks:
        txt = TextClip(
            ...
            color=FONT_COLOR, 
            method="caption", 
            size=(int(VIDEO_W * 0.9), 600), # Sabit geniş kutu, harf kesilmelerini önler
            text_align="center",
            stroke_color="black",
            stroke_width=3
        ).with_duration(chunk_dur).with_start(start_t).with_position(('center', 'center'))
        
        text_clips.append(txt)
        start_t += chunk_dur

    bg_path = os.path.join(local_folder, "bg.mp3")
    bg = AudioFileClip(bg_path).with_effects([afx.AudioLoop(duration=voice_audio.duration)]).with_volume_scaled(0.15)
