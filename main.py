...
FONT_COLOR = "yellow"
FONT_SIZE = 85 # 3 kelime olacağı için biraz daha büyütebiliriz
...
def apply_random_ken_burns(clip):
    # EFEKT SINIRI: %35 ihtimalle efekt uygulama, sabit kalsın
    if random.random() > 0.65:
        return clip

    duration = clip.duration
    zoom_factor = random.uniform(1.15, 1.25) # Hız düşürüldü
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    for chunk in chunks:
        txt = TextClip(
            text=chunk.upper(), # Daha vurucu olması için büyük harf
            font_size=FONT_SIZE, 
            font=VIDEO_FONT, 
            color=FONT_COLOR, 
            method="caption", 
