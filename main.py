...
VIDEO_FONT = "/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf" 
FONT_COLOR = "yellow"
FONT_SIZE = 60 # Kelimeler azaldığı için fontu biraz büyütmek iyi olabilir
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    # --- METİN AKIŞI DÜZENLEME (3 KELİME KURALI) ---
    words = lang_data['script'].split()
    chunks = [" ".join(words[i:i + 3]) for i in range(0, len(words), 3)]
    chunk_duration = voice_audio.duration / len(chunks)
    ...
    for i, chunk in enumerate(chunks):
        t_clip = TextClip(
            ...
            size=(txt_w, None), 
            text_align="center"
        ).with_duration(chunk_duration).with_start(i * chunk_duration).with_position(('center', 'bottom'))
    ...
    # Final video birleşimi: Arka plan videosu + tüm metin klipleri
