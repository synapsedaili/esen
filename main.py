...
# --- GENEL AYARLAR ---
HF_REPO = os.getenv("HF_REPO_ID") # Bu sadece "gokt/esen" olmalı
...
async def main():
    ...
    try:
        print(f"Hugging Face'den {idx} klasörü indiriliyor...")
        # REPO_ID sadece "gokt/esen" olmalı. Pattern olarak doğrudan klasör adını veriyoruz.
        snapshot_download(
            repo_id=HF_REPO, 
            repo_type="dataset", 
            allow_patterns=[f"{idx}/*", f"{idx}/data.json"], 
            token=HF_TOKEN, 
            local_dir=local_dir
        )
        
        # Dosyaların indiği tam yolu bulalım
        folder_path = os.path.join(local_dir, idx)
        
        if not os.path.exists(os.path.join(folder_path, "data.json")):
            # Alternatif: Eğer Hugging Face klasörü boşlukluysa onu da yakalayalım
            found = False
            for d in os.listdir(local_dir):
                if d.strip() == idx:
                    folder_path = os.path.join(local_dir, d)
                    if "data.json" in os.listdir(folder_path):
                        found = True
                        break
            if not found:
                raise Exception(f"Hata: {folder_path} içinde data.json bulunamadı!")
