...
# --- PILLOW ANTIALIAS YAMASI (Hata Çözücü) ---
...
def process_image_to_fill(img_path):
    # Pillow ile resmi açıp yeniden boyutlandırıyoruz (LANCZOS kullanarak)
...
def apply_random_ken_burns(clip):
    ...
    max_x, max_y = int(base_clip.w - VIDEO_W), int(base_clip.h - VIDEO_H)
...
def upload_to_youtube(video_path, meta, lang_code):
    try:
        ...
        if lang_code == "es":
            yt_lang, fixed_tags = "es-ES", ["Crecimiento Personal", "Mentalidad", "Datos del Cerebro", "Verdades Ocultas", "Datos Psicológicos", "Psicología Social", "Por qué somos así."]
        else:
            yt_lang, fixed_tags = "en-US", ["Self Improvement", "Mindset", "Brain Facts", "Hidden Truths", "Psychological Facts", "Social Psychology", "Why We Do What We Do"]
        ...
        res = None
        while res is None: _, res = request.next_chunk()
        print(f"[{lang_code.upper()}] Yüklendi! ID: {res['id']}")
    except Exception as e: print(f"YT {lang_code} Hatası: {e}")
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    clips, current_time, img_dur = [], 0.0, 4.5
    while current_time < voice_audio.duration:
        ...
        clips.append(apply_random_ken_burns(clip))
    ...
    text_clips, start_t = [], 0.0
    for chunk in chunks:
        txt = TextClip(chunk.upper(), fontsize=FONT_SIZE, font=VIDEO_FONT, color=FONT_COLOR, 
                       method="caption", size=(int(VIDEO_W * 0.9), 600), align="Center",
                       stroke_color="black", stroke_width=3).set_duration(chunk_dur).set_start(start_t).set_position(('center', 'center'))
        text_clips.append(txt)
        start_t += chunk_dur

    bg = AudioFileClip(os.path.join(local_folder, "bg.mp3")).fx(afx.audio_loop, duration=voice_audio.duration).volumex(0.15)
...
async def main():
    ...
    with open("current_index.txt", "r") as f: idx = f.read().strip()
    ...
    try:
        ...
        for root, _, files in os.walk(local_dir):
            if root.replace(" ", "").endswith(idx) and "data.json" in files:
                folder_path = root
                break
        if not folder_path: raise Exception(f"{idx} bulunamadı.")
        with open(os.path.join(folder_path, "data.json"), 'r', encoding='utf-8') as f:
            all_data = json.load(f)

        for lang in ["es", "en"]:
            v, m = await create_video_for_lang(lang, all_data, idx, folder_path)
            upload_to_youtube(v, m, lang)
