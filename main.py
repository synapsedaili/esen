...
import random
import re
# Import yapısını en stabil hale getirdik
...
def process_image_to_fill(img_path):
    ...
    if img_aspect_ratio > screen_aspect_ratio:
        ...
        clip = clip.resize(height=VIDEO_H) # resize metodu stabil hale getirildi
...
def apply_random_ken_burns(clip):
    ...
    # Resize ve transform işlemleri moviepy 1.x ve 2.x uyumlu yapıldı
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    for chunk in chunks:
        txt = TextClip(
            chunk.upper(),
            fontsize=FONT_SIZE, # moviepy versiyon uyumu için font_size yerine fontsize
            font=VIDEO_FONT, 
            color=FONT_COLOR, 
            method="caption", 
