...
def process_image_to_fill(img_path):
    # PIL.Image.ANTIALIAS hatası burada LANCZOS ile çözüldü
    ...
    # ANTIALIAS yerine Resampling.LANCZOS kullanımı
...
def upload_to_youtube(video_path, meta, lang_code):
    try:
        ...
        # Dile duyarlı marka etiketleri ve dil ayarları
        if lang_code == "es":
            yt_lang = "es-ES"
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    # 3 kelimelik altyazı sistemi ve ImageMagick hatasına karşı fontsize düzeltmesi
