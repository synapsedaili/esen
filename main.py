...
def process_image_to_fill(img_path):
    """Görseli 9:16 dikey formatı dolduracak şekilde ayarlar."""
...
def apply_random_ken_burns(clip):
    """
    Sıfır Örüntü Mantığı: Her klip için tamamen eşsiz bir hareket vektörü çizer.
    """
    duration = clip.duration
    # Rastgele bir başlangıç büyüklüğü seç (1.2x ile 1.4x arası)
    zoom_factor = random.uniform(1.2, 1.4)
    base_clip = clip.resized(zoom_factor)
    
    # Kameranın gezebileceği alanı hesapla
    max_x = int(base_clip.w - VIDEO_W)
    max_y = int(base_clip.h - VIDEO_H)
    
    # Başlangıç ve bitiş noktalarını tamamen rastgele seç
    # Bu sayede kimi zaman çapraz, kimi zaman sadece dikey/yatay hareket oluşur
    start_x, start_y = random.randint(0, max_x), random.randint(0, max_y)
    end_x, end_y = random.randint(0, max_x), random.randint(0, max_y)

    return base_clip.with_effects([
        vfx.Crop(x1=lambda t: int(start_x + (end_x - start_x) * (t/duration)),
                 y1=lambda t: int(start_y + (end_y - start_y) * (t/duration)),
...
async def create_video_for_lang(lang_code, data, folder_idx, local_folder):
    ...
    while current_time < voice_audio.duration:
        ...
        # Fonksiyon her çağrıldığında kendi içinde random değerlerini üretiyor
